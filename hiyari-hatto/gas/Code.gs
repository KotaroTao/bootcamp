 ...
 * 3. 左メニュー「プロジェクトの設定」（⚙）を開き、
 *    「"appsscript.json" マニフェスト ファイルをエディタで表示する」にチェック
 * 4. エディタに表示された appsscript.json を開き、
 *    同梱の appsscript.json の内容で上書きして保存
 * 5. Code.gs にこのコードを貼り付けて保存
 * 6. 関数「initSheet」を実行（ヘッダー行を自動作成）
 *    - 権限の承認画面が出たら許可する
 * 7. デプロイ → 新しいデプロイ → ウェブアプリ
 *    - 実行するユーザー：自分
 *    - アクセスできるユーザー：全員
 * 8. 表示されたURLをコピーして index.html の GAS_URL に貼り付け
 */
